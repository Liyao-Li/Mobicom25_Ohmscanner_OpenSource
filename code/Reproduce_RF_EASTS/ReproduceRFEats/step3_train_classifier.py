    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % 5 == 0:
            ...
            if results['accuracy'] > best_acc:
                ...
                save_path = 'logs_vae/classifier.pt' if args.use_generator else 'logs/classifier.pt'
                torch.save(model.state_dict(), save_path)

    # test using best model
    # model.load_state_dict(torch.load('logs/classifier.pt'))
    load_path = 'logs_vae/classifier.pt' if args.use_generator else 'logs/classifier.pt'
