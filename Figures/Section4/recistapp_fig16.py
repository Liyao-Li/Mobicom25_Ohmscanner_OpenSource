...
if __name__ == "__main__":
    ...
    if mode ==1:
        ...
        colors = [np.array([0, 114, 189]) / 255, np.array([217, 83, 25]) / 255,
                  ...
                  np.array([237, 177, 32]) / 255, np.array([119, 172, 48]) / 255,np.array([118, 118, 118]) / 255,]
        power0, SCdata0 = np.loadtxt('./data/default_RIO.txt', unpack=True)
        power00, SCdata00 = np.loadtxt('./data/default_Dimsen.txt', unpack=True)

        power, SCdata = np.loadtxt('./data/Exp1_RIO.txt', unpack=True)
        ...
        axes[0].plot(power00, SCdata00, color=blue_rgb, linewidth=line_width, linestyle='-', label='OhmScanner')
        axes[1].plot(power5, SCdata5, color=blue_rgb, linewidth=line_width, linestyle='-', label='OhmScanner')
        axes[2].plot(power6, SCdata6, color=blue_rgb, linewidth=line_width, linestyle='-', label='OhmScanner')
        axes[2].plot(power2, SCdata2, color=red_rgb, linewidth=line_width, linestyle='-', label='RIO')
        axes[3].plot(power7, SCdata7, color=blue_rgb, linewidth=line_width, linestyle='-', label='OhmScanner')
        axes[4].plot(power8, SCdata8, color=blue_rgb, linewidth=line_width, linestyle='-', label='OhmScanner')
        ...
        legend = axes[0].legend(frameon=False, loc=(0.18, 0.52), fontsize=font - 4, handlelength=0.3, ncol=1,
