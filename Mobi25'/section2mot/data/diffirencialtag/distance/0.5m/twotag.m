...
tagID = unique(a(:,5));
% trueID = [2215,2216];
% a=a(ismember(a(:,5),trueID),:);
if length(tagID) == 2 
a(find(a(:,5) == tagID(1)),1);
data_a = a(find(a(:,5) == tagID(1)),:);
count1 = sum(a(:,5)==tagID(1));
data_b = a(find(a(:,5) == tagID(2)),:);
count2 = sum(a(:,5)==tagID(2));
% milisecond
timea = data_a(:,7);
timeb = data_b(:,7);
Timer = max(max(data_a(:,7)),max(data_b(:,7)));
...
fid = fopen(['rotate-40-' num2str(k_tag(kk)) '.txt'], 'a+');%' num2str(ante) '
